...
class TestEMustahsil(unittest.TestCase,Variable):
    ...
    def test_emustahsil_001_indirme_ubl(self):
        ...
        for id in id_values:
            ...
            response = requests.post(url=self.BASE_URL_EMUSTAHSIL + self.DOWNLOAD_UBL, headers=self.headers, data=body_json)
            self.assertIsNotNone(response.json()['data'])
            self.assertIsNone(response.json()['error'])
            tools.write_content_to_zip(response, self.UBL, self.E_MUSTAHSIL)

    def test_emustahsil_002_indirme_html(self):
        ...
        for id in id_values:
            login_request = [{'id': id}]
            body_json = json.dumps(login_request)
            response = requests.post(self.BASE_URL_EMUSTAHSIL + self.DOWNLOAD_HTML, headers=self.headers, data=body_json)
            response_data = response.json()['data']
            self.assertIsNotNone(response_data)
            self.assertIsNone(response.json()['error'])
            tools.write_content_to_zip(response, self.HTML, self.E_MUSTAHSIL)

    def test_emustahsil_003_indirme_pdf(self):
